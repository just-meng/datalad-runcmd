...
```bash
source .venv/bin/activate     # activate dev venv (created by setup below)
...
- **cli.py** — Entry point (`runcmd` command). Parses args, orchestrates the pipeline: load config, find script, extract commands, match positional args to placeholders, resolve, print. Prints resolution info (`{name}: arg -> resolved`) to stderr. Falls back to fuzzy script suggestions when exact match fails.
- **config.py** — Loads `.datalad/runcmd.toml` by walking up from cwd. Defines `Config` and `PlaceholderSpec` dataclasses. `PlaceholderSpec` has no `type` field — behaviour is inferred from which sources are configured.
...
- **resolve.py** — Single unified resolution algorithm for all placeholder types:
  1. Collect candidates from `values`, `file`, and/or `scan_dirs`.
  2. If no candidates → raw substitution (optionally prepend `prefix`).
  3. Filter by `prefix`, score by case-insensitive substring match (`len(arg)/len(candidate)`; exact = 1.0).
  4. Return unique best; raise with full list on ties.
...
skip_header = true
prefix = "sub-"         # filter candidates to those starting with prefix
...
Backward compat: `type = "lookup"/"prefix"/"enum"` is accepted in TOML but ignored — the new inference logic supersedes it.
...
- Matching is case-insensitive; the original candidate casing is preserved in the output.
- File formats: `.tsv` (tab), `.csv` (comma), `.json` (array→elements / object→keys), anything else (one per line). Override with `separator = "..."`.
